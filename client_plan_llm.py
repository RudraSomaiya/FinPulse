...
def _build_retrain_profile_prompt(current_profile: str, edited_reminders: List[Dict[str, str]]) -> str:
    parts = []
    parts.append(
        "You are an expert AI persona architect and writing coach. Your task is to update a 'Writing Style Profile' "
        "based on recent manual edits a human made to reminders generated by an AI.\n"
        "The goal is to LEARN the human's new habits, preferences, and corrections, and MERGE them into the existing "
        "writing profile. You must ADD to or ADJUST the style guidelines, but DO NOT entirely replace or undo "
        "the foundational personality unless explicitly contradicted by the edits. The core persona should remain intact.\n"
        "DO NOT output ANY meta-commentary (e.g., 'Here is the updated profile'). OUTPUT ONLY the complete, final updated "
        "writing style profile document."
    )
    parts.append("\n\nCURRENT WRITING STYLE PROFILE:\n" + current_profile.strip())
    parts.append("\n\nRECENT HUMAN EDITS (LEARNING EXAMPLES):")
    for i, r in enumerate(edited_reminders):
        parts.append(f"\n--- Edit {i+1} ---")
        parts.append(f"Subject: {r.get('subject', '')}")
        parts.append(f"Final Human-Approved Content:\n{r.get('content', '')}")
    
    parts.append(
        "\n\nINSTRUCTIONS:\n"
        "Carefully analyze how the human writes in these recent edits (tone, formatting, length, greetings, sign-offs, phrasing). "
        "Then, logically integrate these insights into the CURRENT WRITING STYLE PROFILE. "
        "If the human edits show a shift in tone (e.g., more casual, shorter, different greetings), update the profile rules "
        "to reflect that. Retain existing rules that don't contradict the new habits."
    )
    return "\n".join(parts)


def retrain_writing_profile(current_profile: str, edited_reminders: List[Dict[str, str]]) -> str:
    """Retrain the core writing profile using recent human edits to reminders.
    
    Uses Gemini 2.5 flash when available, falling back to local Qwen/Ollama model.
    Returns the updated profile text, or the original profile text on failure.
    """
    try:
        if not current_profile or not edited_reminders:
            return current_profile

        prompt = _build_retrain_profile_prompt(current_profile, edited_reminders)
        ...
        print("[client_plan_llm] Gemini unavailable for profile retraining, falling back to local model.")
        text = _call_ollama(prompt)
        if text:
            return str(text).strip()

        print("[client_plan_llm] Both models failed for profile retraining.")
        return current_profile
    except Exception as e:
        print("[client_plan_llm] Unexpected error in retrain_writing_profile:", repr(e))
        return current_profile
